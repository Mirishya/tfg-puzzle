...
const Sketch = () => {
  ...
  const [xOffSliderValue, setXOffSliderValue] = useState(0.01);
  const [yOffSliderValue, setYOffSliderValue] = useState(0.01);
  ...
  const setup = (p) => {
    ...
    p.noiseDetail(1);
      ...
      // Create sliders for xoff and yoff
      const xOffSlider = p.createSlider(0.06, 0.26, xOffSliderValue, 0.01);
      const yOffSlider = p.createSlider(0.06, 0.26, yOffSliderValue, 0.01);
  
      xOffSlider.position(10, p.height + 10);
      yOffSlider.position(10, p.height + 40);
  ...
  const draw = (p) => {
    ...
    let xoff = xOffSliderValue;
   // let xoff = 0;
    for (let x = -p.width / 2; x <= p.width / 2; x += w) {
      let yoff = yOffSliderValue;
      //let yoff = 0;
      for (let y = -p.height / 2; y <= p.height / 2; y += w) {
        const h = p.map(p.noise(xoff + start, yoff + start), 0, 1, -100, 100);
        ...
        yoff+=yOffSliderValue;
        //yoff += 0.06;
      }
      xoff+=xOffSliderValue;
      //xoff += 0.06;
    }
  };
